...
if (!empty($_POST)) {
    // DB更新（アカウント更新）
    $sql = 'UPDATE members SET name=?, password=?, introduction=? WHERE id=?';
    $statement = $db->prepare($sql);
    $statement->execute(
        array(
            $_SESSION['edit']['name'],
            sha1($_SESSION['edit']['password2']),
            $_SESSION['edit']['introduction'],
            $_SESSION['settings']['id']
        )
    );

    header('Location: edit_done.php');
    exit();
};
?>
        ...
        <main>
            ...
            <form action="" method="post">
                ...
                <dl>
                    <dt>名前</dt>
                    <dd>
                        <?php print(htmlspecialchars($_SESSION['edit']['name'], ENT_QUOTES)); ?>
                    </dd>
                    <dt>ID（変更不可）</dt>
                    <dd>
                        <?php print($_SESSION['settings']['id']); ?>
                    </dd>
                    <dt>新しいパスワード</dt>
                    <dd>
                        （表示されません）
                    ...
                    <dt>自己紹介</dt>
                    <dd>
                        <?php print(htmlspecialchars($_SESSION['edit']['introduction'], ENT_QUOTES)); ?>
                    </dd>
                </dl>
                ...
                <div>
                    <a href="edit.php?action=rewrite">&laquo;&nbsp;修正する</a>
                </div>
            </form>
        </main>
    </div>
